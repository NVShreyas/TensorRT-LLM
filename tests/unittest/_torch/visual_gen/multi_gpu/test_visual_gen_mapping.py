...
class TestConstruction:
    ...
    def test_cp_requires_impl(self):
        with pytest.raises(ValueError, match="cp_size > 1 requires cp_impl"):
            VisualGenMapping(world_size=2, rank=0, cp_size=2)

    def test_cp_impl_without_cp_raises(self):
        with pytest.raises(ValueError, match="requires cp_size > 1"):
            VisualGenMapping(world_size=1, rank=0, cp_impl="ring")
...
class TestSingleGPURanksAndGroups:
    ...
    def test_groups_return_single_process_group(self):
        ...
        assert vgm.sp_group is not None
